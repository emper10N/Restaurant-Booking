...
import { ReasonItem } from '../core/models';
...
export class AdminReasonsComponent {
  ...
  create() {
    ...
    this.api.createReason({ ...value, code: value.code.toUpperCase() }).subscribe(() => {
  ...
  toggle(item: ReasonItem) {
    this.api.toggleReasonActive(item.id, !item.active).subscribe(() => this.reload());
  }
